...
return (
  <div className={styles.mainInfo}>
    ...
    <div className={styles.location}>{`Condition: ${weather?.response.ob.weatherPrimary}`}</div>
    <div className={styles.current}>{`${weather?.response.ob.tempC} °C`}</div>
  
    <div className={styles.row}>
    <div className={styles.condition}>{`Wind Speed: ${weather?.response.ob.windSpeedKPH} KPH`}</div>
    <div className={styles.condition}>{`Wind Speed: ${weather?.response.ob.windSpeedMPH} MPH`}</div>
      <div>
        {/* <img
